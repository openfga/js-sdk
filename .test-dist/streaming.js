...
const createAsyncIterableFromReadable = (readable) => {
    return {
        [Symbol.asyncIterator]() {
            ...
            const onEnd = () => {
                if (error)
                    return; // Don't process end if error already occurred
                ended = true;
...
async function* parseNDJSONStream(stream) {
    ...
    if (isString || isBuffer || isUint8Array) {
        const text = isString
            ? stream
            : decoder.decode(isBuffer ? new Uint8Array(stream) : stream);
    ...
    const source = isAsyncIterable ? stream : createAsyncIterableFromReadable(stream);
